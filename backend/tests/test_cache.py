...
def test_cache_basic_operations():
    ...
    assert (
        success or not cache.is_redis_available()
    )  # Allow test to pass if Redis not available
...
def test_user_permission_caching(db_session, client):
    ...
    r = client.post(
        "/api/v1/tenants", json={"name": "Cache Test Corp", "domain": "cache.local"}
    )
    ...
    crud.assign_role_to_user(
        db_session, user["id"], str(role.id), assigned_by=user["id"]
    )
    db_session.commit()
...
def test_cache_invalidation(db_session, client):
    ...
    r = client.post(
        "/api/v1/tenants",
        json={"name": "Cache Invalidate Corp", "domain": "invalidate.local"},
    )
    assert r.status_code == 200
    tenant = r.json()
