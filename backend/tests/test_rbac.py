...
def test_protected_endpoint_allowed_and_forbidden(db_session, client):
    ...
    ra = client.post("/users", json={"email": "alice.rbac@example.com", "password": "pass1234", "client_id": tenant["id"], "first_name": "Alice", "last_name": "RBAC"})
    assert ra.status_code == 200
    alice = ra.json()

    rb = client.post("/users", json={"email": "bob.rbac@example.com", "password": "pass1234", "client_id": tenant["id"], "first_name": "Bob", "last_name": "RBAC"})
    assert rb.status_code == 200
    bob = rb.json()

    la = client.post("/auth/login", params={"email": "alice.rbac@example.com", "password": "pass1234", "client_id": tenant["id"]})
    ...
    lb = client.post("/auth/login", params={"email": "bob.rbac@example.com", "password": "pass1234", "client_id": tenant["id"]})
