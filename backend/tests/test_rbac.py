...
def test_protected_endpoint_allowed_and_forbidden(db_session, client):
    r = client.post("/api/v1/tenants", json={"name": "RBAC Corp", "domain": "rbac.local"})
    assert r.status_code == 200
    tenant = r.json()

    ra = client.post("/api/v1/users", json={"email": "alice.rbac@example.com", "password": "pass1234", "client_id": tenant["id"], "first_name": "Alice", "last_name": "RBAC"})
    assert ra.status_code == 200
    alice = ra.json()

    rb = client.post("/api/v1/users", json={"email": "bob.rbac@example.com", "password": "pass1234", "client_id": tenant["id"], "first_name": "Bob", "last_name": "RBAC"})
    assert rb.status_code == 200
    bob = rb.json()

    la = client.post("/api/v1/auth/login", params={"email": "alice.rbac@example.com", "password": "pass1234", "client_id": tenant["id"]})
    assert la.status_code == 200
    token = la.json()["access_token"]
    headers = {"Authorization": f"Bearer {token}"}

    role_obj = schemas.RoleCreate(name="reader", permissions=["read:protected"], client_id=tenant["id"])
    role_db = crud.create_role(db_session, role_obj)
    crud.assign_role_to_user(db_session, alice["id"], str(role_db.id), assigned_by=alice["id"])
    db_session.commit()

    rprot = client.get("/api/v1/protected/resource", headers=headers)
    assert rprot.status_code == 200

    lb = client.post("/api/v1/auth/login", params={"email": "bob.rbac@example.com", "password": "pass1234", "client_id": tenant["id"]})
