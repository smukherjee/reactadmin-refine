...
def test_auth_misconfiguration_and_broken_access_controls(db_session, client):
    ...
    t1 = client.post(
        "/api/v1/tenants", json={"name": "TenantA", "domain": "a.local"}
    ).json()
    t2 = client.post(
        "/api/v1/tenants", json={"name": "TenantB", "domain": "b.local"}
    ).json()
...
def test_jwt_and_token_hardening(client):
    ...
    t = client.post(
        "/api/v1/tenants", json={"name": "JWTTest", "domain": "jwt.local"}
    ).json()
    client.post(
        "/api/v1/users",
        json={
    ...
    if len(parts) == 3:
        ...
        r = client.get(
            "/api/v1/auth/sessions", headers={"Authorization": f"Bearer {tampered}"}
        )
