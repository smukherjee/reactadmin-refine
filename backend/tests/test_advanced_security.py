...
def check_advanced_dependencies():
    ...
    tools_status = {
        ...
        "beautifulsoup4": {
            "available": False,
            "description": "HTML parsing for XSS detection",
        },
    }
...
def run_basic_owasp_tests():
    ...
    try:
        ...
        report_file = tester.generate_report(
            f"owasp_basic_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
...
def run_advanced_security_tests():
    ...
    try:
        ...
        report_file = tester.generate_advanced_report(
            f"advanced_security_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
...
def run_comprehensive_security_assessment():
    ...
    if "bandit" in available_tools:
        combined_summary["recommendations"].append(
            "✓ Static code analysis available - Review Bandit findings"
        )
    else:
        combined_summary["recommendations"].append(
            "⚠️  Install Bandit for static code analysis"
        )

    if "safety" in available_tools:
        combined_summary["recommendations"].append(
            "✓ Dependency scanning available - Check for vulnerable packages"
        )
    else:
        combined_summary["recommendations"].append(
            "⚠️  Install Safety for dependency vulnerability scanning"
        )

    if "cryptography" in available_tools:
        combined_summary["recommendations"].append(
            "✓ Cryptographic analysis available - Review crypto implementation"
        )
    else:
        combined_summary["recommendations"].append(
            "⚠️  Install cryptography library for crypto analysis"
        )
    ...
    if len(sys.argv) > 1:
        ...
        if command == "check":
            ...
            for tool, info in tools_status.items():
                ...
                version = (
                    f" (v{info.get('version', 'unknown')})" if info["available"] else ""
                )
