...
def test_refresh_tenant_mismatch(db_session, client):
    ...
    ru = client.post('/api/v1/users', json={'email': 'u1@example.com', 'password': 'pass1', 'client_id': ta['id'], 'first_name': 'U', 'last_name': 'One'})
    assert ru.status_code == 200
    u = ru.json()

    # Login to obtain refresh cookie (TestClient will receive cookies)
    L = client.post('/api/v1/auth/login', params={'email': 'u1@example.com', 'password': 'pass1', 'client_id': ta['id']})
...
def test_logout_tenant_mismatch(db_session, client):
    ...
    ru = client.post('/api/v1/users', json={'email': 'lx@example.com', 'password': 'pass1', 'client_id': tx['id'], 'first_name': 'L', 'last_name': 'X'})
    assert ru.status_code == 200
    u = ru.json()

    L = client.post('/api/v1/auth/login', params={'email': 'lx@example.com', 'password': 'pass1', 'client_id': tx['id']})
    assert L.status_code == 200
    sess_id = client.cookies.get('session_id')

    # Authenticate as another tenant's user
    ru2 = client.post('/api/v1/users', json={'email': 'ly@example.com', 'password': 'pass2', 'client_id': ty['id'], 'first_name': 'M', 'last_name': 'Y'})
    assert ru2.status_code == 200
    # login as other tenant user to get current_user in context
    L2 = client.post('/api/v1/auth/login', params={'email': 'ly@example.com', 'password': 'pass2', 'client_id': ty['id']})
    ...
    resp = client.post('/api/v1/auth/logout', params={'session_id': sess_id}, headers=headers)
