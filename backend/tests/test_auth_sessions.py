...
def test_refresh_cookie_rotation_and_session_listing(db_session, client):
    ...
    r = client.post('/tenants', json={'name':'AuthTest','domain':'authtest.local'})
    assert r.status_code == 200
    t = r.json()

    ru = client.post('/users', json={'email':'cookie@example.com','password':'pass1234','client_id': t['id'],'first_name':'C','last_name':'K'})
    assert ru.status_code == 200

    # login should set refresh_token cookie and session_id cookie
    login_resp = client.post('/auth/login', params={'email':'cookie@example.com','password':'pass1234','client_id': t['id']})
    ...
    ls = client.get('/auth/sessions', headers=headers)
    ...
    # logout the session
    lg = client.post('/auth/logout', params={'session_id': session_id}, headers=headers)
    assert lg.status_code == 200
    # after logout, sessions list should be empty
    ls2 = client.get('/auth/sessions', headers=headers)
    assert ls2.status_code == 200
    assert all(s['id'] != session_id for s in ls2.json())


def test_logout_all_revokes_all(db_session, client):
    r = client.post('/tenants', json={'name':'AuthTest2','domain':'authtest2.local'})
    t = r.json()
    ru = client.post('/users', json={'email':'multi@example.com','password':'pass1234','client_id': t['id'],'first_name':'M','last_name':'U'})
    assert ru.status_code == 200

    # login twice to create two sessions
    l1 = client.post('/auth/login', params={'email':'multi@example.com','password':'pass1234','client_id': t['id']})
    ...
    l2 = other.post('/auth/login', params={'email':'multi@example.com','password':'pass1234','client_id': t['id']})
    ...
    r_logout_all = client.post('/auth/logout-all', headers=headers)
    assert r_logout_all.status_code == 200

    # both clients should see no sessions
    ls1 = client.get('/auth/sessions', headers=headers)
    assert ls1.status_code == 200
    assert ls1.json() == []
    token2 = l2.json()['access_token']
    headers2 = {'Authorization': f'Bearer {token2}'}
    ls2 = other.get('/auth/sessions', headers=headers2)
    assert ls2.status_code == 200
    assert ls2.json() == []
