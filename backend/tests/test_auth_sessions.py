...
def test_refresh_cookie_rotation_and_session_listing(db_session, client):
    ...
    r = client.post(
        "/api/v1/tenants", json={"name": "AuthTest", "domain": "authtest.local"}
    )
    ...
    )  # rotation keeps same session row but updates hashes
    ...
    # logout the session
    lg = client.post(
        "/api/v1/auth/logout", params={"session_id": session_id}, headers=headers
    )
...
def test_logout_all_revokes_all(db_session, client):
    r = client.post(
        "/api/v1/tenants", json={"name": "AuthTest2", "domain": "authtest2.local"}
    )
    t = r.json()
    ru = client.post(
        "/api/v1/users",
    ...
    cast(Any, other.app).dependency_overrides.update(
        cast(Any, client.app).dependency_overrides
    )
    l2 = other.post(
        "/api/v1/auth/login",
        params={
