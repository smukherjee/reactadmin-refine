...
class AsyncAuditRepository:
    ...
    async def create(
        ...
        try:
            ...
            logger.info(f"Created audit log {audit_log.id} for action '{action}' by user {user_id}")
            return audit_log
    ...
    async def get_statistics(self, client_id: uuid.UUID) -> Dict[str, Any]:
        ...
        try:
            # Total count
            total_stmt = select(func.count(AuditLog.id)).where(AuditLog.client_id == client_id)
            ...
            recent_stmt = (
                ...
                .where(AuditLog.client_id == client_id, AuditLog.created_at > yesterday)
            )
