...
class AsyncSystemRepository:
    ...
    async def get_system_health(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if (
                ...
                or disk_usage.used / disk_usage.total > 0.95
            ):
                health_data["status"] = "warning"

            logger.info(
                f"System health check: CPU {cpu_percent}%, RAM {memory.percent}%"
            )
            return health_data
    ...
    async def get_database_health(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"Database health check: {connection_time:.2f}ms connection time"
            )
            return db_info
        ...
        try:
            ...
            health_check = {
                ...
                "database": (
                    ...
                    else {"error": str(db_health)}
                ),
                "application": (
                    app_metrics
                    if not isinstance(app_metrics, Exception)
                    else {"error": str(app_metrics)}
                ),
            }

            logger.info(
                f"Health check completed in {total_time:.2f}ms - Status: {overall_status}"
            )
            return health_check
