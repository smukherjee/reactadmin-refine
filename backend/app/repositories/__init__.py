...
from backend.app.models.core import User, Role, UserRole
...
class AsyncUserRepository:
    ...
    async def list_by_tenant(self, client_id: uuid.UUID, skip: int = 0, limit: int = 100) -> List[User]:
    ...
    async def assign_role(
        ...
        try:
            ...
            role_stmt = select(Role).where(Role.id == role_id, Role.client_id == user.client_id)
            role_result = await self.session.execute(role_stmt)
            role = role_result.scalar_one_or_none()
            
            if not role:
                logger.warning(f"Role {role_id} not found or not in same tenant as user {user_id}")
                return False
