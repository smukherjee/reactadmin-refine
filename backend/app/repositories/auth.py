...
from backend.app.models.core import User, Session
...
class AsyncAuthRepository:
    ...
    async def authenticate_user(
        ...
        try:
            ...
            # Create tokens
            access_token = create_access_token({"sub": str(user.id), "client_id": str(client_id)})
            refresh_token = create_refresh_token({"sub": str(user.id), "client_id": str(client_id)})
    ...
    async def refresh_tokens(
        ...
        try:
            ...
            # Create new tokens
            new_access_token = create_access_token({"sub": str(user.id), "client_id": str(client_id)})
            new_refresh_token = create_refresh_token({"sub": str(user.id), "client_id": str(client_id)})
