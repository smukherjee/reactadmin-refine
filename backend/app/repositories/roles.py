...
class AsyncRoleRepository:
    ...
    async def list_by_tenant(self, client_id: uuid.UUID, skip: int = 0, limit: int = 100) -> List[Role]:
    ...
    async def delete(self, role_id: uuid.UUID) -> bool:
        ...
        try:
            ...
            delete_assignments_stmt = delete(UserRole).where(UserRole.role_id == role_id)
    ...
    async def assign_role_to_user(
        ...
        try:
            ...
            if str(user.client_id) != str(role.client_id):
                logger.warning(f"User {user_id} and role {role_id} are not in the same tenant")
                return False
