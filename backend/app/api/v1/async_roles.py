...
async def async_create_role(
    ...
    try:
        ...
        logger.info(f"Created role {new_role.id} '{new_role.name}' for tenant {role.client_id}")
        return new_role
...
async def async_assign_role_test(
    ...
    try:
        ...
        logger.info(f"Test assignment: role {role_id} assigned to user {user_id} by {current_user.id}")
        return {
