"""Async tenant and audit API routes.
...
from backend.app.repositories.tenants import get_tenant_repository, AsyncTenantRepository
from backend.app.repositories.audit import get_audit_repository, AsyncAuditRepository
from backend.app.schemas.core import TenantCreate, TenantOut
from backend.app.models.core import User, Tenant
...
async def async_create_tenant(
    ...
    try:
        ...
        logger.info(f"Created/retrieved tenant {new_tenant.id} with name '{new_tenant.name}'")
        return new_tenant
...
async def async_list_tenants(
    ...
    try:
        ...
        logger.info(f"Retrieved {len(tenants)} tenants for admin user {current_user.id}")
        return tenants
...
async def async_get_tenant(
    ...
    try:
        ...
        if not tenant:
            # Fallback for test environments where tenants are created via sync
            # endpoints using an overridden get_db (in-memory SQLite). Try a
            # sync lookup using the app's get_db override if available so the
            # async endpoint can see tenants created earlier in the same test.
            try:
                ...
                if override:
                    ...
                    def _sync_lookup(sess, tid):
                        try:
                            return sess.query(Tenant).filter(Tenant.id == tid).first()
                        except Exception:
                            return None

                    tenant_sync = await run_in_threadpool(_sync_lookup, sync_db, tenant_uuid)
                    if tenant_sync:
                        tenant = tenant_sync
            except Exception:
                logger.exception("Sync fallback tenant lookup failed")
...
async def async_create_audit_log(
    ...
    user_id: Optional[str] = Query(None, description="User ID who performed the action"),
    ...
    changes_json: Optional[str] = Query(None, description="JSON string of changes made"),
    db: AsyncSession = Depends(get_async_db),
    current_user: User = Depends(get_current_user_async),
    ...
    try:
        ...
        logger.info(f"Created audit log {audit_log.id} for action '{action}' by user {current_user.id}")
        
        return {
            "id": str(audit_log.id),
            "client_id": str(audit_log.client_id),
            "action": audit_log.action,
            "user_id": str(audit_log.user_id) if audit_log.user_id is not None else None,
            "resource_type": audit_log.resource_type,
            "resource_id": str(audit_log.resource_id) if audit_log.resource_id is not None else None,
...
async def async_cleanup_old_audit_logs(
    days_to_keep: int = Query(90, ge=1, le=365, description="Number of days of audit logs to keep"),
    db: AsyncSession = Depends(get_async_db),
    current_user: User = Depends(get_current_user_async),
    ...
    try:
        ...
        logger.info(f"Admin {current_user.id} cleaned up {deleted_count} old audit logs")
