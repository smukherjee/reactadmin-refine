...
class RequestLogger:
    ...
    def log_response(
        ...
        if status_code >= 500:
            ...
            logger.info(
                "HTTP request completed successfully", extra={"extra_fields": log_data}
            )
...
def setup_logging():
    ...
    # use reload_settings() to pick up runtime overrides, with a fallback to module settings.
    try:
        from backend.app.core import config

        runtime_settings = config.reload_settings()
        env = runtime_settings.ENVIRONMENT
...
def log_auth_event(
    ...
    if success:
        ...
        logger.warning(
            f"Auth event failed: {event_type}", extra={"extra_fields": log_data}
        )


def log_permission_check(
    resource: str, action: str, user_id: str, tenant_id: str, allowed: bool
):
...
def log_database_operation(
    operation: str, table: str, duration_ms: float, record_count: int = 1
):
