...
# PID files directory
PIDS_DIR="/tmp/reactadmin_refine"
mkdir -p "$PIDS_DIR"
...
function start_redis_local() {
  if command -v redis-server >/dev/null 2>&1; then
    ...
    nohup redis-server --port 6379 --save "" --appendonly no >"$PIDS_DIR/redis_dev.log" 2>&1 &
    LOCAL_REDIS_PID=$!
...
function start_uvicorn() {
  ...
  if command -v uvicorn >/dev/null 2>&1; then
    nohup uvicorn backend.main:app --reload --host 127.0.0.1 --port "$PORT" >"$PIDS_DIR/uvicorn.log" 2>&1 &
  else
    nohup "$PYTHON_CMD" -m uvicorn backend.main:app --reload --host 127.0.0.1 --port "$PORT" >"$PIDS_DIR/uvicorn.log" 2>&1 &
  fi
  UVICORN_PID=$!
